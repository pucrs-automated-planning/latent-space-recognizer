...
def main(zeros):
    global FPSCLOCK, DISPLAYSURF, BASICFONT, RESET_SURF, RESET_RECT, NEW_SURF, NEW_RECT, ZEROS
    ZEROS = zeros
    #print(zeros)
    pygame.init()
    ...
    mainBoard = generate_random_board()
            ...
            if event.type == MOUSEBUTTONUP:
                ...
                if (spotx, spoty) == (None, None):
                    ...
                    if RESET_RECT.collidepoint(event.pos): # clicked on Reset button
                        ...
                        originalBoard = generate_random_board()
                        mainBoard = getBoardCopy(originalBoard)
                        API.reset_obs()
                        API.reset_demo()
                        int_board = []
...
def drawBoard(board, message):
    ...
    text_left2 = (BOARD_XMARGIN*2) + BOARD_WIDTH*0.9
    text_top2 = 10
    textSurf2, textRect2 = makeText('Candidate Goals:', MESSAGECOLOR, BGCOLOR, text_left2, text_top2)
    ...
    for i in range(len(board)):
        for j in board:
            int_board.append(str(int(j[i])))
    file_name = ''.join(int_board) + '.jpg'
    Z = API.sae._open_image('lightsout-dataset/'+file_name)
    Z = (Z > 0.5) * -1
    board_image = pygame.surfarray.make_surface(Z)
    board_image = pygame.transform.rotate(board_image, -90)
    board_image = pygame.transform.flip(board_image, True, False)
    ...
    DISPLAYSURF.blit(board_image, (BOARD_XMARGIN*1.5, top))
...
def generate_random_board():
    ...
    for i in range(BOARD_COLUMNS):
        ...
        for j in range(BOARD_ROWS):
            board[i].append(bool(random.getrandbits(1)))
    return board
